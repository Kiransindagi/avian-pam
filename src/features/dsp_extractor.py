...
class DSPFeatureExtractor(BaseFeatureExtractor):
    ...
    def extract(self, y: np.ndarray, sr: int) -> Dict[str, float]:
        ...
        min_rms = (
            float(np.min(rms_frame[rms_frame > 1e-7]))
            if np.any(rms_frame > 1e-7)
            else 1e-7
        )
        ...
        flux = (
            np.sqrt(np.sum(np.diff(stft_mag, axis=1) ** 2, axis=0))
        ...
        features["energy_skewness"] = (
            float(stats.skew(rms_frame)) if len(rms_frame) > 2 else 0.0
        )
        features["energy_kurtosis"] = (
            float(stats.kurtosis(rms_frame)) if len(rms_frame) > 2 else 0.0
        )
