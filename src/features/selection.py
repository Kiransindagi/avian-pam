...
class VarianceThresholdSelector(BaseFeatureSelector):
    ...
    def select_features(self, df: pd.DataFrame, target_col: Optional[str] = "bird_count") -> List[str]:
        ...
        logger.info(f"VarianceThresholdSelector: Retained {len(selected)} / {len(feature_cols)} features (threshold={self.threshold}).")
        return selected
...
class CorrelationFilterSelector(BaseFeatureSelector):
    ...
    def select_features(self, df: pd.DataFrame, target_col: Optional[str] = "bird_count") -> List[str]:
        ...
        logger.info(f"CorrelationFilterSelector: Retained {len(selected)} / {len(feature_cols)} features (threshold={self.threshold}).")
        return selected
...
class TreeImportanceSelector(BaseFeatureSelector):
    ...
    def select_features(self, df: pd.DataFrame, target_col: Optional[str] = "bird_count") -> List[str]:
        ...
        if target_col not in df.columns or df[target_col].isnull().all():
            logger.warning("Target column missing for TreeImportanceSelector. Falling back to all features.")
            return feature_cols
        ...
        try:
            ...
            selected = list(importances.sort_values(ascending=False).head(self.top_k).index)
            logger.info(f"TreeImportanceSelector: Selected top {len(selected)} features by Random Forest importance.")
            return selected
        except Exception as e:
            logger.warning(f"TreeImportanceSelector failed: {e}. Returning all features.")
            return feature_cols
...
def get_feature_selector(name: str, **kwargs) -> BaseFeatureSelector:
    ...
    if name not in _SELECTOR_REGISTRY:
        raise KeyError(f"Feature selector '{name}' not found. Available: {list(_SELECTOR_REGISTRY.keys())}")
