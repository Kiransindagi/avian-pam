...
class ModelPlotter:
    ...
    def plot_predictions_vs_actual(self, y_true: np.ndarray, y_pred: np.ndarray, model_name: str):
        """Generates Ground Truth vs Predicted Scatter Plot."""
        out_path = self.figures_dir / "predictions_vs_actual.png"
        fig, ax = plt.subplots(figsize=(7, 6))

        ax.scatter(y_true, y_pred, color="#1f77b4", alpha=0.7, edgecolors="k", s=60, label="Audio Sample Predictions")
        max_val = max(np.max(y_true), np.max(y_pred)) + 1
        ax.plot([0, max_val], [0, max_val], "r--", linewidth=2, label="Ideal Perfect Prediction (y = x)")

        ax.set_title(f"Ground Truth vs Predicted Bird Count ({model_name})", fontsize=12, fontweight="bold")
    ...
    def plot_residuals(self, y_true: np.ndarray, y_pred: np.ndarray, model_name: str):
        ...
        ax.set_title(f"Model Residuals Distribution (Mean Error: {np.mean(residuals):.3f})", fontsize=12, fontweight="bold")
    ...
    def plot_feature_importances(self, importances: Dict[str, float], model_name: str, top_n: int = 15):
    ...
    def plot_leaderboard_comparison(self, df_leaderboard: pd.DataFrame):
        ...
        sns.barplot(x="cv_mae_mean", y="model_name", data=df_leaderboard, palette="mako", ax=ax)
        ax.set_title("Machine Learning Models Out-of-Fold MAE Comparison", fontsize=13, fontweight="bold")
        ax.set_xlabel("Mean Absolute Error (Lower is Better)", fontsize=11)
        ax.set_ylabel("Model Name", fontsize=11)

        for p in ax.patches:
            width = p.get_width()
            ax.annotate(f"{width:.3f}", (width + 0.02, p.get_y() + p.get_height() / 2),
